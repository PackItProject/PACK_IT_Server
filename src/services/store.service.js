...
export const getStoreInfo = async (store_id) => {
    const query = ' SELECT s.* FROM store s WHERE store_id = ?';
...
export const getStoreGrade = async (store_id) => {
    const query = 'SELECT s.store_id, s.store_name, g.nickname, g.grade, g.content, g.image FROM store s JOIN Grade g ON s.store_id = g.store_id WHERE s.store_id = ?';
    const params = [store_id];
    const rows = await pool.execute(query, params);
